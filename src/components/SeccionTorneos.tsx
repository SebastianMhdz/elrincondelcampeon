    ...
    <div className="space-y-5">
      <div className="section-sport-panel rounded-[22px] p-5 md:p-6">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className="flex items-center gap-2 text-lg font-bold text-foreground"><Trophy className="h-5 w-5 text-accent" /> {text.tournamentsTitle}</h2>
            <p className="text-xs text-muted-foreground">{text.tournamentsHint}</p>
          </div>
          {user ? (
            <Button onClick={() => setCreating(true)} className="gap-1.5"><Plus className="h-4 w-4" /> {text.createTournament}</Button>
          ) : (
            <Button variant="outline" onClick={onGoAccount}>{text.signInToCreate}</Button>
          )}
        </div>
        ...
        <div className="mb-4 flex gap-1 rounded-lg border border-border bg-muted/40 p-1">
          {([
            ...
            { id: "open", label: text.filterOpen },
            { id: "mine", label: text.filterMine },
          ] as const).map(f => (
            <button
              key={f.id}
        ...
        {loading ? (
          ...
          <div className="rounded-xl border border-dashed border-border bg-muted/30 p-8 text-center">
            ...
            <p className="text-sm font-semibold text-foreground">{text.noTournamentsToShow}</p>
            <p className="mt-1 text-xs text-muted-foreground">{user ? text.createFirstOne : text.signInToCreateOne}</p>
          </div>
        ) : (
                ...
                <button key={t.id} onClick={() => onSelectTournament(t.id)} className="group rounded-xl border border-border bg-card p-4 text-left transition hover:border-primary hover:shadow-md">
                  ...
                  <div className="mb-2 flex items-start justify-between gap-2">
                    ...
                    <span className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide ${t.signups_open ? "bg-accent/15 text-accent" : t.status === "ongoing" ? "bg-primary/15 text-primary" : "bg-muted text-muted-foreground"}`}>
                      {t.status === "ongoing" ? text.inProgress : t.signups_open ? text.openShort : t.status === "finished" ? text.finished : text.upcoming}
                    </span>
                  </div>
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    ...
                    <p className="flex items-center gap-1 text-muted-foreground"><Users className="h-3 w-3" /> {count}/{t.max_teams}</p>
                  </div>
                  ...
                  <div className="mt-3 flex items-center justify-between">
                    ...
                    {t.signups_open && slotsLeft > 0 && (
                      <span className="text-[10px] font-bold text-accent">{text.freeSlots.replace("{n}", String(slotsLeft))}</span>
                    )}
                  </div>
                </button>
              );
